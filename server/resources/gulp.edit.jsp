...
<tr>
  <th><label for="${bean.targets}">Gulp:</label></th>
  <td>
    <props:selectProperty name="${bean.gulpMode}">
      <c:forEach var="it" items="${bean.gulpModes}">
        <props:option value="${it.value}"><c:out value="${it.title}"/></props:option>
      </c:forEach>
    </props:selectProperty>
    <span class="error" id="error_${bean.gulpMode}"></span>
  </td>
</tr>

<tr>
  <th><label for="${bean.targets}">Gulp File:</label></th>
  <td>
    <props:textProperty name="${bean.file}" className="longField"/>
    <span class="smallNote">
      Specify gulp file path relative to checkout directory.
      Leave blank to use <em>gulpfile.js</em>
    </span>
...
<tr>
  <th><label for="${bean.targets}">Gulp Tasks:</label></th>
  <td>
    <props:multilineProperty name="${bean.targets}" linkTitle="Commands" cols="58" rows="5" expanded="${true}"/>
    <span class="smallNote">Specify gulp tasks to run (new-line separated)</span>
...
<tr>
  ...
  <td>
    ...
    <span class="smallNote">
      Enter additional command line parameters for Gulp. Put each parameter on a new line
      <br/>
